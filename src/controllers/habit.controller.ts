...
export class HabitController {
    ...
    createHabit = async (req: Request, res: Response): Promise<void> => {
        try{
            ...
            res.status(201).json({
                habit, message: "Habit created successfully"
            });
        } catch(error) {
    ...
    updateHabit = async (req: Request, res: Response): Promise<void> => {
        try {
            const habitId = parseInt(req.params.id);
            const updatedHabit = await this.habitService.updateHabit(habitId, req.body);
            res.status(200).json({ updatedHabit, message: "Habit updated successfully" });
        } catch(error) {
    ...
    toggleHabitActive = async (req: Request, res: Response): Promise<void> => {
        try{
            ...
            const updatedHabit = await this.habitService.ToggleHabitActive(habitId, is_active);
            res.status(200).json({ habit: updatedHabit, message: `Habit ${is_active ? "activated" : "deactivated"} successfully`});
        } catch(error) {
    ...
    getHabitsForToday = async (req: Request, res: Response): Promise<void> => {
        try {
          const userId = parseInt(req.params.userId);
          const day = req.query.day as string; // e.g., "Mon"
